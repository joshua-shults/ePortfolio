...
import com.example.weight_tracker.WeightData;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public List<WeightData> getWeightsForUser(String username) {
        List<WeightData> weights = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(
                "SELECT id, weight, date FROM weight_table WHERE username = ? ORDER BY date ASC",
                new String[]{username}
        );
        if (cursor.moveToFirst()){
            do{
                int id = cursor.getInt(0);
                String weightString = cursor.getString(0);
                String date = cursor.getString(1);
                weights.add(new WeightData(weightString, date));
